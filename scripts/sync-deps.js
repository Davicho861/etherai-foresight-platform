...
/*
  ...
  Versión limpia ESM: busca package.json en el árbol cercano, consolida
  dependencies/devDependencies/peerDependencies y actualiza el package.json
  del directorio de trabajo (crea backup). Diseñado para ejecutarse antes de
  `npm ci` en entornos de build.
*/
...
async function main() {
  ...
  console.log('[sync-deps] Starting dependency consolidation in', cwd);
  ...
  if (packageJsonPaths.length === 0) {
    console.log('[sync-deps] No package.json files found. Nothing to do.');
    return 0;
  }
  ...
  try {
    ...
    console.log('[sync-deps] Wrote consolidated package.json to', targetPath);
  } catch (e) {
    console.error('[sync-deps] Failed to write package.json:', e.message);
    process.exit(1);
  }

  // Attempt a quick regenerate of package-lock if npm is available
  try {
    execSync('npm ci --ignore-scripts --no-audit --no-fund', { stdio: 'inherit', cwd });
    console.log('[sync-deps] npm ci executed to regenerate lockfile.');
  } catch (e) {
    console.warn('[sync-deps] npm ci failed or was skipped:', e.message);
  }

  console.log('[sync-deps] Done.');
  return 0;
}

main().catch(err => {
  console.error('[sync-deps] Unexpected error:', err);
  process.exit(1);
});